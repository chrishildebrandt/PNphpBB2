...
function PNphpBB2_MultiBlockblock_display($blockinfo)
{
	...
	if ($vars['display_posts'])
	{
		...
		$blockinfo['content'] .= "<table align=\"center\" width=\"98%\" cellpadding=\"1\" cellspacing=\"0\" border=\"0\">";
                ...
                $style_code .= '<!--'; 
                $style_code .= '.underline'; 
                $style_code .= '{border-bottom: 1px solid #' . $vars['line_color'] . '; padding: 1px 3px px 3px; margin: 1px 1px 1px 1px;}'; 
                $style_code .= '//-->'; 
                $style_code .= '</style>'; 
                PageUtil::addVar('rawtext', $style_code); 
		...
		if (($blockinfo['position'] == 'center' || $blockinfo['position'] == 'c' || $blockinfo['position'] == 't' || $blockinfo['position'] == 'b') && !$vars['last_X_scroll'])
		{
		...
		while (list($topic_subject, $post_replies, $post_views, $topic_id, $post_username, $post_id, $poster_id, $post_time, $post_subject, $forum_id, $forum_name, $cat_title) = $result->fields)
		{
			...
			if ($blockinfo['position'] == 'center' || $blockinfo['position'] == 'c' || $blockinfo['position'] == 't' || $blockinfo['position'] == 'b')
			{
				if ($vars['last_X_scroll'] && $ie_marquee)
				{
...
// 			$blockinfo['content'] .= "<td align=\"left\"><span class=\"pn-normal\"><strong>" . _PD . "</strong></span></td>";
	...
	if ($vars['display_category'])
	{
		...
		$catcount = 0;
		$numcolumns = ($blockinfo['position'] == 'center' || $blockinfo['position'] == 'c' || $blockinfo['position'] == 't' || $blockinfo['position'] == 'b') ? 3 : 1;
	...
	if ($vars['display_members'] && (!($blockinfo['position'] == 'center') || !($blockinfo['position'] == 'c') || !($blockinfo['position'] == 't') || !($blockinfo['position'] == 'b')))
	{
...
// 				$blockinfo['content'] .= '<tr><td align="center">' . _NOMEMBERS . '</td></tr></table>';
		...
		if (!pnUserLoggedIn())
		{
			...
			$f_url = pnModURL('Users', 'user', 'register'); 
                        $blockinfo['content'] .= '<tr><td align="center"><img src="' . $phpbb_root_path . 'pnimages/' . $vars['info_icon'] . '" alt="" /><br />' . pnML('_YOUAREANON', array('u' => $f_url), true) . '</td></tr></table></td></tr>';
			if ($vars['display_login'])
			{
				$f_action = pnModURL('Users', 'user', 'login'); 
                                $blockinfo['content'] .= '<tr><td align="center"><b>' . _USERNAME.'</b></td></tr>'; 
				...
				pnModLangLoad('Users', 'user'); 
                                $f_remember = _USERS_REMEMBERME; 
				$blockinfo['content'] .= '<tr><td align="center"><input type="submit" value="' . _LOGIN . '" />'; 
                                $blockinfo['content'] .= '<input type="hidden" name="authid" value="'.pnSecGenAuthKey('Users').'" />'; 
				$blockinfo['content'] .= '<input type="hidden" name="url" value="'.pnGetCurrentURL().'" /></td></tr></table></form></td></tr>';
				}
				$blockinfo['content'] .= '<tr><td align="center"><input type="submit" value="' . _LOGIN . '">';
				$blockinfo['content'] .= '<input type="hidden" name="module" value="Users">';
