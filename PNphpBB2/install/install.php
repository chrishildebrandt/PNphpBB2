...
$update_config = array(
	'board_email'	=> $GLOBALS['PNConfig']['DBInfo']['default']('adminmail'),
...
$sql = "SELECT pn_pass FROM " . $GLOBALS['PNConfig']['DBInfo']['default']('prefix') . "_users WHERE pn_uid='2'";
...
$sql_query .= "UPDATE " . $table_prefix . "users SET username = '" . str_replace("\'", "''", $admin_name) . "', user_password='" . str_replace("\'", "''", $admin_pass_md5) . "', user_lang = '" . str_replace("\'", "''", $language) . "', user_email='" . str_replace("\'", "''", $GLOBALS['PNConfig']['DBInfo']['default']('adminmail')) . "' WHERE username = 'Admin';";
...
if (defined('PN_VERSION_NUM') && !strcmp(PN_VERSION_NUM, "0.8.", 4)) {
	...
	pnModAPIFunc('Modules', 'admin', 'setstate', array('id' => $id, 'state' => _PNMODULE_STATE_INACTIVE));
