...
$dbms=$GLOBALS['PNConfig']['DBInfo']['default']('dbtype');

$dbhost = $GLOBALS['PNConfig']['DBInfo']['default']('dbhost');
$dbuser = $GLOBALS['PNConfig']['DBInfo']['default']('dbuname');
$dbpasswd = $GLOBALS['PNConfig']['DBInfo']['default']('dbpass');
$dbname = $GLOBALS['PNConfig']['DBInfo']['default']('dbname');
...
if (defined("PHPBB2_TO_PNPHPBB2") && !($errored) )
{
	...
	$sql_query = "DELETE FROM " . $GLOBALS['PNConfig']['DBInfo']['default']('prefix') . "_users WHERE pn_uid > 2;";
	$result = evaluate_statement($sql_query, $lang['PNphpBB2_Delete_Users']);
	 
	// Delete user id's from the group membership table
	$sql_query = "DELETE FROM " . $GLOBALS['PNConfig']['DBInfo']['default']('prefix') . "_group_membership WHERE pn_uid > 2;";
	...
	while (list($user_id, $username, $user_email, $user_website, $user_regdate, $user_icq, $user_occ, $user_from, $user_interests, $user_viewemail, $user_aim, $user_yim, $user_msnm, $user_password, $user_timezone) = $result->fields)
	{
		...
		$sql_query = "INSERT INTO $pntable[users] (pn_uid, pn_uname, pn_email,
									...
									VALUES ('" . pnVarPrepForStore($user_id) . "', '" . pnVarPrepForStore($username) . "',
													...
													'".pnVarPrepForStore($GLOBALS['PNConfig']['DBInfo']['default']('storyhome'))."','',0,0,0,'',0,'','',
													'".pnVarPrepForStore($GLOBALS['PNConfig']['DBInfo']['default']('commentlimit'))."', '0', '".pnVarPrepForStore($user_timezone + 12 )."')";
		...
		else
		{
			...
			$sql_query2 = "SELECT $column[gid]
											FROM $pntable[groups]
											WHERE $column[name]='". $GLOBALS['PNConfig']['DBInfo']['default']('defaultgroup') . "'";
